...
export default function TopArticles({ items }: { items: ContentItem[] }) {
  return (
    <div className="rounded-xl border border-[#1a2540] bg-[#121a2a] p-4">
      ...
      {items.length === 0 ? (
        <p className="text-xs text-[#475569]">No articles collected yet.</p>
      ) : (
        <div className="space-y-2">
          {items.map((item) => (
            <a
              ...
              className="block rounded-lg border border-[#1a2540] bg-[#0f1520] p-3 transition-all hover:border-violet-500/30 hover:bg-[#0f1520]/80"
            >
              <p className="text-xs font-medium text-[#e2e8f0] leading-relaxed line-clamp-2">
                {item.title}
              </p>
              {item.summary && (
                <p className="mt-1 text-[10px] text-[#475569] line-clamp-1">
                  {item.summary}
                </p>
              )}
